    ...
    def get(self, request, *args, **kwargs):
        try:
            purchase_order_list = PurchaseOrderInvoice.objects.filter(
                sender__user=request.user, is_invoiced=False
            )
            po_list = []
...
class MerchantInvoiceListView(RoleRequiredMixin, StoreRequiredMixin, View):
    required_role = "Merchant"

    def get(self, request, *args, **kwargs):
        try:
            purchase_order_list = PurchaseOrderInvoice.objects.filter(
                sender__user=request.user, is_invoiced=True
            )
            invoice_list = []
            for each in purchase_order_list:
                order_item = each.order_item
                ...
                invoice_list.append(
                    {
                        "invoice_id": each.pk,
                        "invoice_date": (
                            str(each.invoice_date.date()) if each.invoice_date else ""
                        ),
                        "invoice_no": order_item.tracking_id,
                        "vendor_name": vendor_name,
                        "vendor_company_name": vendor_company_name,
                        "vendor_payment_amount": float(vendor_payment_amount),
                        "merchant_profit_amount": float(merchant_profit_amount),
            ...
            return render(
                request,
                "merchant/billing/invoice_list.html",
                {
            ...
            logger.error(traceback.format_exc())
            messages.error(request, "Error while listing Invoice.")
            return render(
                request,
                "merchant/error.html",
                {"message": "Error while listing Invoice."},
                status=500,
            )
...
class MerchantInvoiceDetailView(RoleRequiredMixin, StoreRequiredMixin, View):
    required_role = "Merchant"

    def get(self, request, invoice_id, *args, **kwargs):
        try:
            po = PurchaseOrderInvoice.objects.get(pk=invoice_id)
            order_item = po.order_item
            total_amount = float(order_item.total_amount)
            merchant_profit_amount = order_item.merchant_margin_price * int(
                order_item.quantity
            ...
            order_detail_data = {
                ...
                "tracking_id": order_item.tracking_id,
                "invoice_id": po.pk,
                "invoice_date": str(po.invoice_date.date()) if po.invoice_date else "",
                "order_date": str(order_item.parent.created_at.date()),
                "product": order_item.product.inventory.product.name,
                "product_description": order_item.product.inventory.product.description,
                "vendor_price": order_item.vendor_price,
            ...
            return render(
                request,
                "merchant/billing/invoice_detail.html",
                {
                    "data": order_detail_data,
                },
            )
...
class InvoicePaymentView(RoleRequiredMixin, StoreRequiredMixin, View):
    required_role = "Merchant"
    ...
    def get(self, request, invoice_id, *args, **kwargs):
        try:
            ...
            po.save()
            return redirect("merchant_invoice_list")

        except Exception:
            logger.error(traceback.format_exc())
            messages.error(request, "Error while updating Invoice.")
            return render(
                request,
                "vendor/error.html",
                {"message": "Error while updating updating Invoice."},
                status=500,
            )
...
class VendorPurchaseOrderListView(RoleRequiredMixin, CompanyRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            purchase_order_list = PurchaseOrderInvoice.objects.filter(
                reciever__user=request.user, is_invoiced=False
            )
            po_list = []
            for each in purchase_order_list:
                order_item = each.order_item
...
class VendorInvoiceListView(RoleRequiredMixin, CompanyRequiredMixin, View):
    required_role = "Vendor"

    def get(self, request, *args, **kwargs):
        try:
            purchase_order_list = PurchaseOrderInvoice.objects.filter(
                reciever__user=request.user, is_invoiced=True
            )
            po_list = []
            for each in purchase_order_list:
                order_item = each.order_item
            ...
            return render(
                request,
                "vendor/billing/invoice_list.html",
                {
                    "po_list": po_list,
                },
            )
...
class VendorInvoiceDetailView(RoleRequiredMixin, CompanyRequiredMixin, View):
    required_role = "Vendor"
    ...
    def get(self, request, invoice_id, *args, **kwargs):
        try:
            ...
            return render(
                request,
                "vendor/billing/invoice_detail.html",
                {
                    "data": order_detail_data,
                },
            )
